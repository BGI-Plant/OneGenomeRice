...
本任务旨在利用OGR基础模型的表征学习能力，在全基因组尺度上对亚种来源进行精细推断，从而识别 籼稻（*Oryza sativa* subsp. *indica*）与粳稻（*Oryza sativa* subsp. *japonica*）之间的血缘渗入。与传统依赖 SNP 统计或局部序列比对的方法不同，本研究直接从原始基因组序列出发，基于OGR模型提取高维嵌入并构建下游预测模型，以在序列层面捕获深层遗传结构差异，辅助识别亚种间潜在血缘渗入区段。

## **2. 数据来源与处理**

本研究使用来源明确、注释完善的高质量水稻组装基因组，包括：

数据基础：一组高质量组装水稻基因组

亚群划分：依据RiceVarMap数据库的亚群分类标签

样本选取：选取与3KRGP（3K rice genome project）重叠的样本，再经全基因组变异主成分分析（PCA）进一步筛选；最终在籼稻与温带粳稻小组中各选取 10 份代表性样本，以在保留亚群内遗传多样性的同时，尽量减少育种历史中血缘渗入带来的干扰。此外从籼稻与温带粳稻中各另选 1 份代表性样本，构建独立测试集用于评价。

## **3. 任务设计**

### **(1) 总体框架**

模型以OGR基础模型为底座。与常规微调不同，本研究**不更新**基础模型参数，而是对每个 8 kb 序列直接提取嵌入，并在此基础上构建轻量级下游预测模型，核心流程如下：

![总体框架与流程](images/Introgression_Analysis-a.png)

**数据构建与划分**：收集籼稻与温带粳稻的全基因组序列，在**个体层面**划分为训练集与测试集（比例约 10:1）。

**基因组窗口切分**：将每条基因组划分为固定长度滑动窗口（8 kb），得到覆盖全基因组的序列片段集合。

**序列表征提取**：使用OGR对每个 8 kb 片段编码，得到高维嵌入表示。

**下游建模**：在嵌入上训练随机森林，学习从序列表征到亚群归属概率的映射（$P_{\mathrm{indica}}$：属于籼稻的概率；$P_{\mathrm{japonica}}$：属于粳稻的概率）。
...
| **测试集** | **分类器** | **ACC** | **AUC** |
|:---:|:---:|:---:|:---:|
| 1 温带粳稻 + 1 籼稻 | 随机森林(n_estimators=100) | 0.804 | 0.794 |
...
我们尝试将该框架用于分析精英粳稻品种盐丰 47（YF47）中的籼稻血缘渗入；该品系代表历史上亚种间杂交育种形成的育种系。这些区段多呈连续区块而非孤立位点，表明血缘渗入在区段尺度上被捕获，与育种过程中相邻基因组片段一并发生血缘渗入的现象一致。
...
3. **训练用 JSONL 格式**  
   ...
   **字段说明：**
   ...
   | `label` | 2 元整数数组 | 多标签向量：`[japonica_binary, indica_binary]` | `[1, 0]`（纯粳稻）或 `[0, 1]`（纯籼稻） |
...
| `prob_japonica` | 浮点数 | 粳稻来源概率（0.0–1.0） |
| `prob_indica` | 浮点数 | 籼稻来源概率（0.0–1.0） |
| `group` | 字符串 | 分类结果：`japonica`（粳稻概率≥0.8，且籼稻概率<0.8）、`indica`（籼稻概率≥0.8，且粳稻稻概率<0.8）、`introgressed`（相反情况）或 `ambiguous` |
