...
def resume_metrics(y_pred, y_score, y_test, num_classes, dataset, layer, classifer_type, config):
    ...
    if config.dataset_info[dataset]["eval_task"] == "classification" or config.dataset_info[dataset]["eval_task"] == "labels":
        ...
        #print(f"y_true shape: {y_true.shape}, y_score shape: {y_score.shape}, y_pred shape: {y_pred.shape}")
        #print(f"y_true sample: {y_true[:5]}, y_score sample: {y_score[:5]}, y_pred sample: {y_pred[:5]}")
...
def train_xgboost_classifier(X_train, y_train, X_test, X_val, y_val, config, dataset_name, layer, random_state=42):
    X_train_np = catter(
    ...
    if config.dataset_info[dataset_name]["eval_task"] == "classification":
        ...
        preds_list = []
        probs_list = []
        print(f"Training {n_labels} XGBoost binary classifiers (one per label)...")

        for i in range(n_labels):
            print(f"  Training classifier for label {i}...")
            ...
            xgb = XGBClassifier(
                n_estimators=config["xgb_n_estimators"],
                random_state=random_state,
                eval_metric=config["xgb_eval_metric"],      # e.g., 'logloss' or 'auc'
                learning_rate=config["xgb_learning_rate"],
                max_depth=config["xgb_max_depth"],
                subsample=0.8,              # 行采样防过拟合
                colsample_bytree=0.6,       # 列采样（关键！避免被1024维噪声淹没）
                reg_alpha=1.0,              # L1 正则
                reg_lambda=1.0,             # L2 正则
                #tree_method='gpu_hist',
                early_stopping_rounds=50,
            )
            #early_stopping = EarlyStopping(rounds=50, metric_name='logloss', data_name='validation_0')
            xgb.fit(X_train_np, y_train_i, eval_set=[(X_train_np, y_train_i), (X_val_np, y_val_i)], verbose=True)
            models.append(xgb)

            pred_i = xgb.predict(X_test_np)                # shape: (n_samples,)
            prob_i = xgb.predict_proba(X_test_np)[:, 1]    # probability of class=1

            preds_list.append(pred_i)
            probs_list.append(prob_i)

        # Stack results
        preds = np.column_stack(preds_list)   # (n_samples, n_labels)
        probs = np.column_stack(probs_list)   # (n_samples, n_labels), P(label=1)

        # Save all models together
        if config['xgb_save_model']:
            os.makedirs(
                f"{config['eval_result_path']}/{config['model_name']}/last_epoch_model", exist_ok=True)
            joblib.dump(models, f"{config['eval_result_path']}/{config['model_name']}/last_epoch_model/{dataset_name}-{layer}layer.xgb.pkl")
            print(f"All {n_labels} models saved")

        print("Multi-label XGBoost training completed.")
...
def train_rf_classifier(X_train, y_train, X_val, y_val, device, config, dataset_name, layer, n_estimators=100, random_state=42):
    X_train_np = catter(
        X_train, config["pooled_embeddings_cat_dim"]).cpu().numpy()
    y_train_np = y_train.cpu().numpy()
    X_val_np = catter(X_val, config["pooled_embeddings_cat_dim"]).cpu().numpy()
    ...
    if config.dataset_info[dataset_name]["eval_task"] == "classification":
        ...
        for i in range(n_labels):
            ...
            rf = RandomForestClassifier(
                ...
                random_state=random_state, 
                max_features=0.2,       # ⬆️ 从 'sqrt'(32) → 205
                max_depth=15,           # ⬇️ 防过深
            ...
            pred_i = rf.predict(X_val_np)
            prob_i = rf.predict_proba(X_val_np)[:, 1]  # 概率为正类的概率
        ...
        # Save all models together
        if config['rf_save_model']:
            os.makedirs(
                f"{config['eval_result_path']}/{config['model_name']}/last_epoch_model", exist_ok=True)
            joblib.dump(rf_models, f"{config['eval_result_path']}/{config['model_name']}/last_epoch_model/{dataset_name}-{layer}layer.rf.pkl")
            print(f"All {n_labels} models saved")
...
def evaluate_model_on_dataset_layer(dataset_name, gpu_id, model_name, layer, config):
    ...
    if classifer_type == "MLP":
        ...
        y_pred, y_score = train_rf_classifier(
            X_train, y_train, X_eval, y_eval, device, config, dataset_name, layer, n_estimators=config['rf_n_estimators'], random_state=config['seed'])
    elif classifer_type == "XGB":
        y_pred, y_score = train_xgboost_classifier(
            X_train, y_train, X_test, X_eval, y_eval, config, dataset_name, layer, random_state=config['seed'])
